...
extension UICollectionView {
    public func registerCell(forNib nibClass: UICollectionViewCell.Type) {
        register(
            ...
            forCellWithReuseIdentifier: String(describing: nibClass)
        )
    }
    ...
    public func registerView(
        forNib nibClass: UICollectionReusableView.Type,
        flowSupplementaryKind: FlowSupplementaryKind? = nil
    ) {
        guard let kind = flowSupplementaryKind else {
            register(
                UINib(nibName: String(describing: nibClass), bundle: Bundle(for: nibClass)),
                forSupplementaryViewOfKind: String(describing: nibClass),
    ...
    public func registerView(
        forClass anyClass: UICollectionReusableView.Type,
        flowSupplementaryKind: FlowSupplementaryKind? = nil
    ) {
        guard let kind = flowSupplementaryKind else {
            register(
                anyClass,
                forSupplementaryViewOfKind: String(describing: anyClass),
